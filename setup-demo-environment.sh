...
readonly CONTAINER_NAME="grafana-khulnasoft-plugin-demo"
readonly CONTAINER_PORT=3000
readonly GRAFANA_DOCKER_IMAGE="grafana/grafana:10.2.2"
readonly KHULNASOFT_PLUGIN_REPO="khulnasoft/khulnasoft-grafana-datasource-plugin"
...
function get_latest_plugin() {
  curl -s https://api.github.com/repos/${KHULNASOFT_PLUGIN_REPO}/releases/latest \
    | jq -r '.assets[] | select(.name|match("zip$")) | .browser_download_url'
}

function run() {
  echo "get the latest khulnasoft plugin"
  declare plugin_url
...
function grafana_url() {
  ...
  echo "Grafana with khulnasoft plugin is available under: http://${endpoint} use admin/admin credentials to get access."
}
...
$PROGNAME spin up Grafana instance with khulnasoft datasource plugin.

OPTIONS
  run     run grafana docker container with khulnasoft datasource plugin
